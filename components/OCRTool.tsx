...
const OCRTool: React.FC<OCRToolProps> = ({ onSave }) => {
  ...
  const handleAiRefine = async () => {
    ...
    try {
      ...
      setDocs(prev => prev.map(d => d.id === selectedDocId ? { ...d, status: 'error' } : d));
    } finally {
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      await errorService.createError({
        ...
        model: 'All Models',
        title: extractedData.title || `Lỗi ${extractedData.code}`,
        status: 'active',
        severity: 'medium',
        symptom: extractedData.symptom,
        cause: extractedData.cause,
        components: [],
        steps: extractedData.steps
      });

      alert(`Đã lưu mã lỗi ${extractedData.code} vào hệ thống!`);
      if (onSave) onSave();
    } catch (e) {
        ...
        <div className="flex h-10 items-center rounded-xl bg-background-dark p-1 border border-border-dark/50">
          {['Tài liệu', 'Soạn thảo', 'Trích xuất'].map((label, i) => (
            <button
              key={i}
              onClick={() => setTab(i)}
              className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${tab === i ? 'bg-surface-dark text-white shadow-md' : 'text-text-secondary hover:text-white'
                }`}
            >
              {i + 1}. {label}
            </button>
              ...
              <section className="bg-surface-dark border border-border-dark/30 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
                ...
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-text-secondary uppercase">Mã lỗi & Hãng</label>
                    <div className="flex gap-2">
                      ...
                      <input type="text" placeholder="Hãng (VD: Panasonic)" value={extractedData.brand} onChange={e => setExtractedData({ ...extractedData, brand: e.target.value })} className="w-40 bg-background-dark border-border-dark rounded-xl px-4 py-3 text-white" />
