...
const SystemUpdate: React.FC = () => {
  ...
  const checkUpdates = async () => {
    ...
    try {
      const update = await systemService.checkForUpdates();
      setUpdateAvailable(update);
      addLog(`Tìm thấy phiên bản mới: ${update.version}`);
    } catch (e) {
      addLog("Không tìm thấy bản cập nhật mới hoặc lỗi kết nối.");
    } finally {
      setIsChecking(false);
    }
  };
  ...
  const PAST_VERSIONS = [
    { version: '3.2.0', date: '10/05/2024', notes: ['Initial Release of Admin Pro', 'Added AI Assistant Beta'] },
    { version: '3.1.5', date: '01/05/2024', notes: ['Fixed Login layout issues', 'Improved dark mode contrast'] },
    { version: '3.1.0', date: '20/04/2024', notes: ['Added Brand Manager module', 'Integrated Firebase Auth'] },
  ];
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">Cập nhật hệ thống</h1>
        <div className="bg-surface-dark border border-border-dark rounded-xl p-1 flex">
          <button
            onClick={() => setActiveTab('update')}
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'update' ? 'bg-primary text-white shadow-lg' : 'text-text-secondary hover:text-white'}`}
          >
            Cập nhật
          </button>
          <button
            onClick={() => setActiveTab('history')}
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'history' ? 'bg-primary text-white shadow-lg' : 'text-text-secondary hover:text-white'}`}
          >
            Lịch sử
          </button>
        </div>
      ...
      {activeTab === 'update' ? (
        ...
        <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
          {PAST_VERSIONS.map((ver, idx) => (
            <div key={idx} className="bg-surface-dark border border-border-dark/50 rounded-2xl p-6 hover:border-primary/30 transition-all">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-surface-dark border border-border-dark flex items-center justify-center text-text-secondary">
                    <span className="material-symbols-outlined">history</span>
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-white">Phiên bản {ver.version}</h3>
                    <p className="text-xs text-text-secondary">Đã cập nhật: {ver.date}</p>
                  </div>
                </div>
                <span className="text-green-500 text-xs font-bold uppercase tracking-widest bg-green-500/10 px-3 py-1 rounded-full">Success</span>
              </div>
              <div className="space-y-1 pl-14">
                {ver.notes.map((note, nIdx) => (
                  <p key={nIdx} className="text-sm text-text-secondary flex items-start gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-border-dark mt-1.5"></span>
                    {note}
                  </p>
                ))}
              </div>
